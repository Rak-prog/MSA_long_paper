function h = plot_results(T, titname)
    ...
    idx = find(contains(labs_y,'SessoM')); 
    if ~isempty(idx); labs_y(idx) = {'SexMvsF'}; end
    idx = find(contains(labs_y,'SottotipoMotorioP')); 
    if ~isempty(idx); labs_y(idx) = {'PhenotypePvsC'}; end
    idx = find(contains(labs_y,'TimeT1:UMSARSIcovcentered')); 
    if ~isempty(idx); labs_y(idx) = {'TimeFUPvsBL x USMARSI'}; end
    idx = find(contains(labs_y,'TimeT1:UMSARSIIcovcentered')); 
    if ~isempty(idx); labs_y(idx) = {'TimeFUPvsBL x USMARSII'}; end
    idx = find(contains(labs_y,'MCIs1no01')); 
    if ~isempty(idx); labs_y(idx) = {'MCIYESvsNO'}; end
    idx = find(contains(labs_y,'TimeT1')); 
    if ~isempty(idx); labs_y(idx) = {'TimeFUPvsBL'}; end
    idx = find(contains(labs_y,'UMSARSIcovcentered')); 
    if ~isempty(idx); labs_y(idx) = {'USMARSI'}; end
    idx = find(contains(labs_y,'UMSARSIIcovcentered')); 
    if ~isempty(idx); labs_y(idx) = {'USMARSII'}; end
